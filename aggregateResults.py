##############################################################################
# Created by Cormac Keogh ITT Dublin 2016
#

# Python Script to agregate results posted to CSV files from YCSB cient and create an aggregate CSV file
#
    ...
    with open(inFile, "r") as ifile:
        for line in ifile:
            ...
            if(i == 2):
                ...
                if(line.find('cmd')==-1):  # continue looking
                    if(line.find('[READ]')!=-1):
                        operation = "READ"
...
outfileNameDesc = configName + '_' + workload + '_' + str(threads) + '_' + str(target) + '_DT' +curDateTime + '.dsc'

# copy the scenario description
#for line in open('CurrentScenario.txt'):
#    csvOutfile.write(line)

shutil.copy2('CurrentScenario.txt', outfileNameDesc)
...
csvOutfile.close()
oscommand = "type CurrentScenario.txt  >> " + outfileName

os.system( oscommand)

shutil.copy2(outfileName, 'x:')
